<?php
    ...
    if(isset($_POST['name'])){
        ...
        $query = mysqli_query($con, "insert into pass(name, email, contact, date, dest, password) values ('$name', '$email', '$contact', '$date', '$dest', '$password')");
		$lid = mysqli_insert_id($con);
		
		$nod = round((strtotime($date) - time())/(60*60*24))+1;
		$result = mysqli_query($con, "select price from destination where name = '$dest'");
    }
?>
